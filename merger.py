...
class Merger(tf.keras.layers.Layer):
    ...
    def __init__(self, cfg):
        ...
        self.layer1 = tf.keras.Sequential([
            tf.keras.layers.Conv3D(9, 16, kernel_size=3, padding=1),
        ...
        self.layer2 = tf.keras.Sequential([
            tf.keras.layers.Conv3D(16, 8, kernel_size=3, padding=1),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(0.2)
        ])
        # self.layer2 = tf.keras.Sequential()
        ...
        self.layer3 = tf.keras.Sequential([
            tf.keras.layers.Conv3D(8, 4, kernel_size=3, padding=1),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(0.2)
        ])
        # self.layer3 = tf.keras.Sequential()
        ...
        self.layer4 = tf.keras.Sequential([
            tf.keras.layers.Conv3D(4, 2, kernel_size=3, padding=1),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(0.2)
        ])
        # self.layer4 = tf.keras.Sequential()
    ...
    def forward(self, raw_features, coarse_volumes):
        ...
        for i in range(n_views_rendering):
            raw_feature = tf.squeeze(raw_features[i], dim=1)
