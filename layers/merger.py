...
class Merger(tf.keras.layers.Layer):
    ...
    def __init__(self, args, **kwargs):
        ...
        self.layer1.add(tf.keras.layers.Conv3D(
        ...
        self.layer2.add(tf.keras.layers.Conv3D(
            8, kernel_size=3, padding="same"))
        self.layer2.add(tf.keras.layers.BatchNormalization())
        self.layer2.add(tf.keras.layers.LeakyReLU(0.2))

        self.layer3 = tf.keras.Sequential()
        self.layer3.add(tf.keras.layers.Conv3D(
            4, kernel_size=3, padding="same"))
        ...
        self.layer4.add(tf.keras.layers.Conv3D(
            2, kernel_size=3, padding="same"))
        ...
        self.layer5.add(tf.keras.layers.Conv3D(
            2, kernel_size=3, padding="same"))
    ...
    def call(self, raw_features, coarse_volumes):
        ...
        volume_weights = tf.stack(volume_weights).transpose(
            volume_weights, (1, 0, 2, 3, 4))
