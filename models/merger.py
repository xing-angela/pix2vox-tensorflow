...
class Merger(tf.keras.Model):
    def __init__(self, cfg, **kwargs):
        ...
        self.layer2 = tf.keras.models.Sequential([
            tf.keras.layers.Conv3D(filters=8, kernel_size=3, padding='same'),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(alpha=cfg.NETWORK.LEAKY_VALUE)
        ])
        self.layer3 = tf.keras.models.Sequential([
            tf.keras.layers.Conv3D(filters=4, kernel_size=3, padding='same'),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(alpha=cfg.NETWORK.LEAKY_VALUE)
        ])
        self.layer4 = tf.keras.models.Sequential([
            tf.keras.layers.Conv3D(filters=2, kernel_size=3, padding='same'),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(alpha=cfg.NETWORK.LEAKY_VALUE)
        ])
        self.layer5 = tf.keras.models.Sequential([
            tf.keras.layers.Conv3D(filters=1, kernel_size=3, padding='same'),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(alpha=cfg.NETWORK.LEAKY_VALUE)
        ])
