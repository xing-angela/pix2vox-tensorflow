    ...
    def load_data(self):
        for file in self.file_list:
            ...
            for path in image_paths:
                rendering_image = cv2.imread(path, cv2.IMREAD_UNCHANGED).astype(np.float32)/255.
                if len(rendering_image.shape) <3:
                    print('[WARN] %s It seems the image file %s is grayscale.' % (dt.now(), path))
        ...
        self.images = np.asarray(rendering_images)
        self.volumes = np.asarray(self.volumes)

        print('[INFO] %s Complete loading files of the dataset.' % (dt.now()))

        # return taxonomy_name, sample_name, rendering_images, self.volumes, self.bounding_boxes
